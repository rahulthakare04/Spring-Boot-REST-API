package com.SpringBoot.RestAPI.dto;

import com.SpringBoot.RestAPI.annotations.EmployeeRoleValidation;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.validation.constraints.*;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

import java.time.LocalDate;

@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
public class EmployeeDTO {

    private Long id;

    @NotBlank(message = "Name of the employee can not be null")
    @Size(min = 3,max = 10,message = "number of character in name should be in the range :[3,10]")
    private String name;

    @Email(message = "Email should be valid email")
    @NotBlank(message = "Email of the employee can not be null")
    private String email;

    @NotNull(message = "age of the employee can not be null")
    @Max(value = 80,message = "Age of the employee can not be grater than 80 ")
    @Min(value = 18,message = "Age of the employee can not be less than 18")
    private Integer age;

    @NotBlank(message = "role of the employee can not be null")
    //@Pattern(regexp ="^(ADMIN|USER)$",message = "role of the employee either be USER or ADMIN")
    @EmployeeRoleValidation
    private String role;

    @NotNull(message = "salary of the employee can not be null")
    @Positive(message = "salary of the employee should be positive")
    @Digits(integer = 6,fraction = 2,message = "the Salary can be in the form of XXXXXX.YY")
    @DecimalMax(value = "10000.99")
    @DecimalMin(value = "100.50")
    private Double salary;

    @PastOrPresent(message = "dateOfJoining joining can not be in future ")
    private LocalDate dateOfJoining;

    @AssertTrue(message = "employee should be active")
    @JsonProperty("isActive")
    private Boolean isActive;


}
